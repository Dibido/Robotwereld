...
RobotWorldCanvas::RobotWorldCanvas(Window* anOwner) :
		ScrolledCanvas(anOwner, DEFAULT_ID), popupPoint( DefaultPosition), startActionPoint(
		DefaultPosition), endActionPoint( DefaultPosition), actionOffset(
		DefaultPosition), startActionSize( DefaultSize), startActionShape(
				nullptr), endActionShape(nullptr), selectedShape(nullptr), activationEnabled(
				false), selectionEnabled(false), menuItemEnabled(false), dandEnabled(
				true), notificationHandler(nullptr) {
	initialise();
}
...
RobotWorldCanvas::RobotWorldCanvas(Window* anOwner,
		...
		ScrolledCanvas(anOwner, DEFAULT_ID), ViewObject(aModelObject), popupPoint(
		DefaultPosition), startActionPoint( DefaultPosition), endActionPoint(
		DefaultPosition), actionOffset( DefaultPosition), startActionSize(
		DefaultSize), startActionShape(nullptr), endActionShape(nullptr), selectedShape(
				nullptr), activationEnabled(false), selectionEnabled(false), menuItemEnabled(
				false), dandEnabled(true), notificationHandler(nullptr) {
	initialise();
}
/**
 *
...
Point RobotWorldCanvas::devicePointFor(const Point& aScreenPoint) const {
	Point devicePoint;
	/*owner->getCanvas()->*/CalcUnscrolledPosition(aScreenPoint.x,
			aScreenPoint.y, &devicePoint.x, &devicePoint.y);
	return devicePoint;
}
/**
 *
...
Point RobotWorldCanvas::screenPointFor(const Point& aDevicePoint) const {
	Point screenPoint;
	CalcScrolledPosition(aDevicePoint.x, aDevicePoint.y, &screenPoint.x,
			&screenPoint.y);
	return screenPoint;
}
/**
...
void RobotWorldCanvas::enableLeftDownHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_LEFT_DOWN,
				wxMouseEventHandler(RobotWorldCanvas::OnLeftDown));
	} else {
...
void RobotWorldCanvas::enableLeftUpHandling(bool enable /* = true */) {
	if (enable == true) {
		...
		Disconnect(wxEVT_LEFT_UP,
				wxMouseEventHandler(RobotWorldCanvas::OnLeftUp));
	}
}
/**
 *
...
void RobotWorldCanvas::enableLeftDClickHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_LEFT_DCLICK,
				wxMouseEventHandler(RobotWorldCanvas::OnLeftDClick));
	} else {
...
void RobotWorldCanvas::enableMiddleDownHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_MIDDLE_DOWN,
				wxMouseEventHandler(RobotWorldCanvas::OnMiddleDown));
	} else {
...
void RobotWorldCanvas::enableMiddleUpHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_MIDDLE_UP,
				wxMouseEventHandler(RobotWorldCanvas::OnMiddleUp));
	} else {
...
void RobotWorldCanvas::enableMiddleDClickHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_MIDDLE_DCLICK,
				wxMouseEventHandler(RobotWorldCanvas::OnMiddleDClick));
	} else {
...
void RobotWorldCanvas::enableRightDownHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_RIGHT_DOWN,
				wxMouseEventHandler(RobotWorldCanvas::OnRightDown));
	} else {
...
void RobotWorldCanvas::enableRightUpHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_RIGHT_UP,
				wxMouseEventHandler(RobotWorldCanvas::OnRightUp));
	} else {
...
void RobotWorldCanvas::enableRightDClickHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_RIGHT_DCLICK,
				wxMouseEventHandler(RobotWorldCanvas::OnRightDClick));
	} else {
...
void RobotWorldCanvas::enableMouseMotionHandling(bool enable /* = true */) {
	if (enable == true) {
		Connect(wxEVT_MOTION,
				wxMouseEventHandler(RobotWorldCanvas::OnMouseMotion));
	} else {
...
void RobotWorldCanvas::enableKeyHandling(bool enable /* = true */) {
	if (enable == true) {
		...
		KeyEventHandler(RobotWorldCanvas::OnKeyDown));
		Disconnect(wxEVT_CHAR,
				wxCharEventHandler(RobotWorldCanvas::OnCharDown));
	}
}
/**
 *
...
void RobotWorldCanvas::setSelectedShape(ShapePtr aSelectedShape) {
	...
	if (previousSelectedShape && aSelectedShape
			&& previousSelectedShape->getObjectId()
					!= aSelectedShape->getObjectId()) {
...
void RobotWorldCanvas::setRobotWorld(Model::RobotWorldPtr aRobotWorld) {
	setModelObject(
			std::dynamic_pointer_cast < Model::ModelObject > (aRobotWorld));
}
/**
 *
 */
...
void RobotWorldCanvas::initialise() {
	...
	notificationHandler =
			new Base::NotificationHandler<std::function<void(NotifyEvent&)> >(
					[this](NotifyEvent& anEvent) {this->OnNotificationEvent(anEvent);});
...
void RobotWorldCanvas::render(wxDC& dc) {
	...
	if (startActionShape != nullptr && actionStatus == DRAWING) {
		...
		dc.DrawLine(startActionShape->getCentre().x,
				startActionShape->getCentre().y, endActionPoint.x,
				endActionPoint.y);
	}
}
/**
...
void RobotWorldCanvas::handleLeftUp(MouseEvent& event) {
	RectangleShapePtr startRectangleShape = std::dynamic_pointer_cast
			< RectangleShape > (startActionShape);
	RectangleShapePtr endRectangeShape = std::dynamic_pointer_cast
			< RectangleShape > (getShapeAt(event.GetPosition()));
	...
	case DRAWING: {
		if (startRectangleShape && endRectangeShape) {
			ShapePtr lineShape(
					new LineShape(startRectangleShape, endRectangeShape));
	...
	if (Model::RobotWorld::getRobotWorld().isCommunicating()) {
		...
		if (worldptr) {
			...
			if (Application::MainApplication::isArgGiven("-remote_ip")) {
				remoteIpAdres = Application::MainApplication::getArg(
						"-remote_ip").value;
			}
			if (Application::MainApplication::isArgGiven("-remote_port")) {
				remotePort = Application::MainApplication::getArg(
						"-remote_port").value;
			}
	...
	Model::RobotWorld::getRobotWorld().setChanged();
...
void RobotWorldCanvas::handleMouseMotion(MouseEvent& event) {
	...
	if (event.Moving() == false && event.Dragging() == true
			&& startActionShape != nullptr) {
		int tolerance = 2;
...
void RobotWorldCanvas::handleEndDrag(ShapePtr aShape) {
	RobotShapePtr robotShape = std::dynamic_pointer_cast < RobotShape
			> (aShape);
	if (robotShape) {
	...
	WayPointShapePtr wayPointShape = std::dynamic_pointer_cast < WayPointShape
			> (aShape);
	if (wayPointShape) {
		wayPointShape->getWayPoint()->setPosition(wayPointShape->getCentre(),
				false);
		return;
	}
	// Handle the RectangleShapes that are part of a wall
	RectangleShapePtr rectangleShape = std::dynamic_pointer_cast
...
void RobotWorldCanvas::handleAddRobot(CommandEvent& UNUSEDPARAM(event)) {
	RobotShapePtr robot(
			new RobotShape(
					Model::RobotWorld::getRobotWorld().newRobot("Robot",
							popupPoint)));
	addShape(robot);
	Refresh();
}
...
void RobotWorldCanvas::handleEditRobot(CommandEvent& UNUSEDPARAM(event)) {
	RobotShapePtr shape = std::dynamic_pointer_cast < RobotShape
			> (getSelectedShape());
	if (shape) {
		std::string name = GetTextFromUser("The new Robot name:",
				"Enter a new name");
...
void RobotWorldCanvas::handleAddWayPoint(CommandEvent& UNUSEDPARAM(event)) {
	WayPointShapePtr wayPoint(
			new WayPointShape(
					Model::RobotWorld::getRobotWorld().newWayPoint("Joost",
							popupPoint)));
	addShape(wayPoint);
	Refresh();
}
...
void RobotWorldCanvas::handleEditWayPoint(CommandEvent& UNUSEDPARAM(event)) {
	WayPointShapePtr shape = std::dynamic_pointer_cast < WayPointShape
			> (getSelectedShape());
	if (shape) {
		std::string name = GetTextFromUser("The new WayPoint name:",
				"Enter a new name");
...
void RobotWorldCanvas::handleAddGoal(CommandEvent& UNUSEDPARAM(event)) {
	GoalShapePtr goal(
			new GoalShape(
					Model::RobotWorld::getRobotWorld().newGoal("Goal",
							popupPoint)));
	addShape(goal);
	Refresh();
}
...
void RobotWorldCanvas::handleEditGoal(CommandEvent& UNUSEDPARAM(event)) {
	GoalShapePtr shape = std::dynamic_pointer_cast < GoalShape
			> (getSelectedShape());
	if (shape) {
		std::string name = GetTextFromUser("The new Goal name:",
				"Enter a new name");
...
void RobotWorldCanvas::handleAddWall(CommandEvent& UNUSEDPARAM(event)) {
	...
	ShapePtr wall(
			new WallShape(
					Model::RobotWorld::getRobotWorld().newWall(
							start->getCentre(), end->getCentre(), false), start,
					end));
...
void RobotWorldCanvas::handleDeleteWall(CommandEvent& UNUSEDPARAM(event)) {
	WallShapePtr shape = std::dynamic_pointer_cast < WallShape
			> (getSelectedShape());
	if (shape) {
		removeShape(shape);
	}
 ...
 */
void RobotWorldCanvas::handleNotification(
		NotifyEvent& UNUSEDPARAM(aNotifyEvent)) {
	remove<Model::Robot, View::RobotShape>(
			Model::RobotWorld::getRobotWorld().getRobots());
	add<Model::Robot, View::RobotShape>(
			Model::RobotWorld::getRobotWorld().getRobots());

	remove<Model::WayPoint, View::WayPointShape>(
			Model::RobotWorld::getRobotWorld().getWayPoints());
	add<Model::WayPoint, View::WayPointShape>(
			Model::RobotWorld::getRobotWorld().getWayPoints());

	remove<Model::Goal, View::GoalShape>(
			Model::RobotWorld::getRobotWorld().getGoals());
	add<Model::Goal, View::GoalShape>(
			Model::RobotWorld::getRobotWorld().getGoals());

	remove<Model::Wall, View::WallShape>(
			Model::RobotWorld::getRobotWorld().getWalls());
	add<Model::Wall, View::WallShape>(
			Model::RobotWorld::getRobotWorld().getWalls());
...
void RobotWorldCanvas::handleMenu(const Point& UNUSEDPARAM(aScreenPoint)) {
	...
	Connect(ID_ADD_ROBOT, wxEVT_COMMAND_MENU_SELECTED,
	CommandEventHandler(RobotWorldCanvas::OnAddRobot));

	popupMenu.Append(ID_ADD_WAYPOINT, _T("Add waypoint"),
			_T("ID_ADD_WAYPOINT2"));
	Connect(ID_ADD_WAYPOINT, wxEVT_COMMAND_MENU_SELECTED,
	CommandEventHandler(RobotWorldCanvas::OnAddWayPoint));

	popupMenu.Append(ID_ADD_GOAL, _T("Add goal"), _T("ID_ADD_GOAL2"));
	Connect(ID_ADD_GOAL, wxEVT_COMMAND_MENU_SELECTED,
	CommandEventHandler(RobotWorldCanvas::OnAddGoal));

	popupMenu.Append(ID_ADD_WALL, _T("Add wall"), _T("ID_ADD_WALL2"));
	Connect(ID_ADD_WALL, wxEVT_COMMAND_MENU_SELECTED,
	CommandEventHandler(RobotWorldCanvas::OnAddWall));

	popupMenu.Append(ID_WORLD_INFO, _T("World info"), _T("ID_WORLD_INFO2"));
	Connect(ID_WORLD_INFO, wxEVT_COMMAND_MENU_SELECTED,
	CommandEventHandler(RobotWorldCanvas::OnWorldInfo));
...
void RobotWorldCanvas::handleItemMenu(ShapePtr aSelectedShape,
	...
	if (std::dynamic_pointer_cast < RobotShape > (aSelectedShape)) {
		...
		Connect(ID_EDIT_ROBOT, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnEditRobot));
		popupMenu.Append(ID_DELETE_ROBOT, _T("Delete robot"),
				_T("ID_DELETE_ROBOT"));
		Connect(ID_DELETE_ROBOT, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnDeleteRobot));
	} else if (std::dynamic_pointer_cast < GoalShape > (aSelectedShape)) {
		popupMenu.Append(ID_EDIT_GOAL, _T("Edit goal"), _T("ID_EDIT_GOAL"));
		Connect(ID_EDIT_GOAL, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnEditGoal));
		popupMenu.Append(ID_DELETE_GOAL, _T("Delete goal"),
				_T("ID_DELETE_GOAL"));
		Connect(ID_DELETE_GOAL, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnDeleteGoal));
	} else if (std::dynamic_pointer_cast < WayPointShape > (aSelectedShape)) {
		popupMenu.Append(ID_EDIT_WAYPOINT, _T("Edit waypoint"),
				_T("ID_EDIT_WAYPOINT"));
		Connect(ID_EDIT_WAYPOINT, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnEditWayPoint));
		popupMenu.Append(ID_DELETE_WAYPOINT, _T("Delete waypoint"),
				_T("ID_DELETE_WAYPOINT"));
		Connect(ID_DELETE_WAYPOINT, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnDeleteWayPoint));
	} else if (std::dynamic_pointer_cast < WallShape > (aSelectedShape)) {
		popupMenu.Append(ID_DELETE_WALL, _T("Delete wall"),
				_T("ID_DELETE_WALL"));
		Connect(ID_DELETE_WALL, wxEVT_COMMAND_MENU_SELECTED,
		CommandEventHandler(RobotWorldCanvas::OnDeleteWall));
	} else {
		Application::Logger::log("Huh?: " + aSelectedShape->asDebugString());
	}

	popupMenu.Append(ID_SHAPE_INFO, _T("Show info"), _T("ID_SHAPE_INFO"));
	Connect(ID_SHAPE_INFO, wxEVT_COMMAND_MENU_SELECTED,
	CommandEventHandler(RobotWorldCanvas::OnShapeInfo));
...
void RobotWorldCanvas::addShape(WallShapePtr aWallShape) {
	...
	RectangleShapePtr start(
			new RectangleShape(aWallShape->getWall()->getPoint1()));
	RectangleShapePtr end(
			new RectangleShape(aWallShape->getWall()->getPoint2()));
...
void RobotWorldCanvas::removeShape(RobotShapePtr aRobotShape) {
	...
	Model::RobotWorld::getRobotWorld().deleteRobot(aRobotShape->getRobot(),
			false);
...
void RobotWorldCanvas::removeShape(WayPointShapePtr aWayPointShape) {
	aWayPointShape->stopHandlingNotificationsFor(
			*aWayPointShape->getWayPoint());
	Model::RobotWorld::getRobotWorld().deleteWayPoint(
			aWayPointShape->getWayPoint(), false);
...
void RobotWorldCanvas::removeGenericShape(ShapePtr aShape) {
	if (isShapeSelected()
			&& aShape->getObjectId() == getSelectedShape()->getObjectId()) {
...
void RobotWorldCanvas::OnWorldInfo(CommandEvent& UNUSEDPARAM(event)) {
	Application::Logger::log(
			Model::RobotWorld::getRobotWorld().asDebugString());
}
/**
 *
 */
