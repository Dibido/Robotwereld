...
bool MainApplication::OnInit()
{
	...
	if (MainApplication::isArgGiven("-worldname"))
	{
		...
		frame = new MainFrameWindow(
				"RobotWorld : " + MainApplication::getArg("-worldname").value);
...
// argv[0] contains the executable name as one types on the command line (with or without extension)
	MainApplication::commandlineArguments.push_back(
			CommandlineArgument(0, "Executable", argv[0]));
		...
		if (currentArg[0] == '-') // ordinary argument
		{
			...
			for (size_t j = 0; j < argLength; ++j)
			{
				if (currentArg[j] == '=')
				{
					...
					MainApplication::commandlineArguments.push_back(
							CommandlineArgument(i, variable, value));
					inserted = true;
				}
			}
...
{
	std::vector<CommandlineArgument>::iterator i = std::find(
			MainApplication::commandlineArguments.begin(),
			MainApplication::commandlineArguments.end(), aVariable);
	return i != commandlineArguments.end();
}

/* static */CommandlineArgument& MainApplication::getArg(
...
/* static */CommandlineArgument& MainApplication::getArg(
		unsigned long anArgumentNumber)
{
